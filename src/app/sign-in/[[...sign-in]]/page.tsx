...
// rendering one directly into JSX throws "Objects are not valid as a React
// child". This extracts a safe, guaranteed-string message regardless of
// whether we're handed a string, a Clerk error object, a native Error, or
...
export default function SignInPage() {
  ...
  const identifierErrorMessage = toErrorMessage(errors?.fields?.identifier);
  ...
  return (
   ...
   <div className="flex flex-[2] items-start justify-center overflow-y-auto px-6 py-10 sm:items-center sm:py-6 lg:flex-[3]">
        <div className="w-full max-w-sm">
          ...
          {globalErrorMessage && (
            <p className="mb-3 text-xs text-red-600">{globalErrorMessage}</p>
          )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1">
              ...
              {identifierErrorMessage && (
                <p className="text-xs text-red-600">{identifierErrorMessage}</p>
              )}
            </div>
